...
from dataset import CRIGDatasetFolderWithLabel
...
def log_images(model, batch, N, unconditional_guidance_scale):
    ...
    if unconditional_guidance_scale > 1.0:
        ...
        samples_cfg, _ = model.sample_log(cond={"c_concat": [c_cat], "c_crossattn": [c], "c_label": [c_label]},
...
if __name__=="__main__":
    ...
    if "max_len" in args.controlnet:
        dataset = CRIGDatasetFolderWithLabel(data_root,data_json=os.path.join(data_root,args.controlnet.data_json),
                                             max_len=args.controlnet.max_len,resolution=args.controlnet.resolution)
    else:
        dataset = CRIGDatasetFolderWithLabel(data_root,data_json=os.path.join(data_root,args.controlnet.data_json),
                                             resolution=args.controlnet.resolution)
    dataFilter = []
